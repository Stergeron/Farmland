...
io.on('connection', function(socket) {
  ...
  socket.on("buyland", function() {
    if (farms[name].gold >= 30) {
      farms[name].gold -= 30;
      var newlength = {
        cols: farms[name].farm[0].length + 2,
        rows: farms[name].farm.length + 2
      };
      farms[name].farm.push([]);
      for (var r = 0; r < newlength.rows.length; r++) {
        for (var c = 0; c < newlength.cols.length; c++) {
          if (farms[name].farm[r][c] === undefined) {
