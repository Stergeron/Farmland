...
io.on('connection', function(socket) {
	...
	socket.on("buy", function(index) {
		...
		if (listing !== undefined && farms[name].gold > listing.price && listing.quantity > 0) {
			...
			if (!found) farms[name].inventory.push({
				plant: listing.plant,
				quantity: 1
			});
	...
	socket.on("market", function(item) {
		farms[name].inventory.forEach(function(i) {
			if (item.plant.hash == i.plant.hash) {
				...
				if (!found) marketListings.push({
					plant: item.plant,
					price: item.plant.yield + 15,
					seller: name,
					quantity: 1
				});
				socket.emit("update", farms[name]);
	...
	socket.on("buyland", function(cb) {
		if (farms[name].gold >= farms[name].farm.length * 15) {
			farms[name].gold -= farms[name].farm.length * 15;
			var newlength = {
				cols: farms[name].farm[0].length + 1,
				rows: farms[name].farm.length + 1
			};
			farms[name].farm.push([]);
			for (var r = 0; r < newlength.rows; r++) {
				console.log("Pew");
				while(farms[name].farm[newlength.rows-1] === undefined) {
					farms[name].farm.push([]);
				}
				var c = farms[name].farm[0].length-1;
				while(farms[name].farm[r][newlength.cols -1] === undefined) {
					farms[name].farm[r].push({
						row: r,
						col: c,
						plant: {}
					});
